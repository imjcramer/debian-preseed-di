...
[ ! -e /etc/skel/.config/labwc/xinitrc ] || {
  printf "fatal: X11 xinit asset must not be staged for Labwc: %s\n" /etc/skel/.config/labwc/xinitrc >&2
  exit 1
}
if grep -q "dbus-run-session" /usr/local/bin/labwc-greeter-session /usr/local/bin/labwc-session; then
  fatal "desktop wrappers must use the dbus-broker user bus, not dbus-run-session"
fi
if grep -q "allWorkspaces" /etc/skel/.config/labwc/rc.xml; then
  fatal "Labwc rc.xml must use NextWindow/PreviousWindow workspace attributes instead of deprecated windowSwitcher allWorkspaces"
fi
grep -q "<action name=\"NextWindow\" workspace=\"all\" />" /etc/skel/.config/labwc/rc.xml || {
  fatal "Labwc rc.xml must keep Alt-Tab cycling across all workspaces with NextWindow workspace=all"
}
grep -q "<action name=\"PreviousWindow\" workspace=\"all\" />" /etc/skel/.config/labwc/rc.xml || {
  fatal "Labwc rc.xml must keep reverse Alt-Tab cycling across all workspaces with PreviousWindow workspace=all"
}
grep -q "LIBSEAT_BACKEND=seatd" /usr/local/bin/labwc-greeter-session || {
  fatal "greeter wrapper must force the seatd libseat backend"
}
grep -q "^export GIO_USE_VFS=local\$" /usr/local/bin/labwc-greeter-session || {
  fatal "greeter wrapper must use local GIO VFS to avoid greeter GVFS daemons"
}
grep -q "/usr/bin/cage -s -m last -- /usr/bin/gtkgreet -s /etc/greetd/gtkgreet.css" /usr/local/bin/labwc-greeter-session || {
  fatal "greeter wrapper must launch gtkgreet through Cage without layer-shell mode"
}
if grep -q "import-environment" /usr/local/bin/labwc-autostart ||
   grep -q "dbus-update-activation-environment" /usr/local/bin/labwc-autostart; then
  fatal "Labwc autostart must leave activation environment updates to labwc"
fi
grep -q "labwc-session.target" /usr/local/bin/labwc-autostart || {
  fatal "Labwc autostart must start labwc-session.target for user services"
}
if grep -q "gvfs-daemon.service" /etc/skel/.config/labwc/shutdown; then
  fatal "Labwc shutdown hook must not ask systemd to kill GVFS daemon cgroups"
fi
grep -q "^BindsTo=graphical-session.target\$" /etc/skel/.config/systemd/user/labwc-session.target || {
  fatal "Labwc session target must bind to graphical-session.target"
}
...
grep -q "hyprpolkitagent.service" /usr/local/bin/labwc-autostart || {
  fatal "Labwc autostart must start the Hypr polkit user service"
}
grep -q "/usr/libexec/hyprpolkitagent" /usr/local/bin/labwc-autostart || {
  fatal "Labwc autostart must keep the Debian Hypr polkit direct executable fallback"
}
verify_session_dropin() {
  ...
  grep -q "^After=graphical-session.target labwc-session.target\$" "$dropin" || {
    fatal "user session drop-in must order after the Labwc graphical session: ${unit}"
  }
  grep -q "^PartOf=graphical-session.target labwc-session.target\$" "$dropin" || {
    fatal "user session drop-in must stop with the Labwc graphical session: ${unit}"
  }
  grep -q "^ExecCondition=/usr/bin/systemctl --user --quiet is-active graphical-session.target\$" "$dropin" || {
    fatal "user session drop-in must be gated on the active Labwc graphical session: ${unit}"
  }
}

verify_session_dropin hyprpolkitagent.service
grep -q "^Environment=QT_NO_XDG_DESKTOP_PORTAL=1\$" /etc/skel/.config/systemd/user/hyprpolkitagent.service.d/10-labwc-session.conf || {
  fatal "Hypr polkit agent must not recursively register as a host portal client"
}
...
do
  ...
  if [ -r "/usr/lib/systemd/user/${portal_unit}" ]; then
    ...
    grep -q "^ConditionEnvironment=WAYLAND_DISPLAY\$" "$portal_dropin" || {
      fatal "portal user unit must be skipped until WAYLAND_DISPLAY is available: ${portal_unit}"
    }
    grep -q "^PartOf=graphical-session.target labwc-session.target\$" "$portal_dropin" || {
      fatal "portal user unit must stop with the Labwc graphical session: ${portal_unit}"
    }
    grep -q "^ExecCondition=/usr/bin/systemctl --user --quiet is-active graphical-session.target\$" "$portal_dropin" || {
      fatal "portal user unit must be gated on the active Labwc graphical session: ${portal_unit}"
    }
  fi
done
if [ -r /usr/lib/systemd/user/xdg-desktop-portal-lxqt.service ]; then
  ...
  grep -q "^Environment=QT_NO_XDG_DESKTOP_PORTAL=1\$" "$lxqt_portal_dropin" || {
    fatal "LXQt portal backend must not recursively register as a host portal client"
  }
fi
if [ -r /usr/share/dbus-1/services/org.freedesktop.impl.portal.desktop.kwallet.service ]; then
  ...
  grep -q "^Name=org.freedesktop.impl.portal.desktop.kwallet\$" "$kwallet_portal_service" || {
    fatal "KWallet portal backend D-Bus override has the wrong service name"
  }
  grep -q "^Exec=/usr/bin/env QT_NO_XDG_DESKTOP_PORTAL=1 /usr/bin/ksecretd\$" "$kwallet_portal_service" || {
    fatal "KWallet portal backend must not recursively register as a host portal client"
  }
fi
grep -Eq "^WSDD_PARAMS=\"(-i [A-Za-z0-9_.-]+( -i [A-Za-z0-9_.-]+)*)?\"\$" /etc/wsdd-server/defaults || {
  fatal "wsdd defaults must contain only an empty value or explicit -i interface arguments"
}
...
account_labwc_rc="$account_home/.config/labwc/rc.xml"
if grep -q "allWorkspaces" "$account_labwc_rc"; then
  fatal "primary account Labwc rc.xml must not use deprecated windowSwitcher allWorkspaces"
fi
grep -q "<action name=\"NextWindow\" workspace=\"all\" />" "$account_labwc_rc" || {
  fatal "primary account Labwc rc.xml must keep Alt-Tab cycling across all workspaces"
}
grep -q "<action name=\"PreviousWindow\" workspace=\"all\" />" "$account_labwc_rc" || {
  fatal "primary account Labwc rc.xml must keep reverse Alt-Tab cycling across all workspaces"
}
...
verify_account_session_dropin() {
  ...
  grep -q "^After=graphical-session.target labwc-session.target\$" "$dropin" || {
    fatal "primary account user session drop-in must order after the Labwc graphical session: ${unit}"
  }
  grep -q "^PartOf=graphical-session.target labwc-session.target\$" "$dropin" || {
    fatal "primary account user session drop-in must stop with the Labwc graphical session: ${unit}"
  }
  grep -q "^ExecCondition=/usr/bin/systemctl --user --quiet is-active graphical-session.target\$" "$dropin" || {
    fatal "primary account user session drop-in must be gated on the active Labwc graphical session: ${unit}"
  }
}

verify_account_session_dropin hyprpolkitagent.service
grep -q "^Environment=QT_NO_XDG_DESKTOP_PORTAL=1\$" "$account_home/.config/systemd/user/hyprpolkitagent.service.d/10-labwc-session.conf" || {
  fatal "primary account Hypr polkit agent must not recursively register as a host portal client"
}
...
[ ! -e "$account_home/.config/labwc/xinitrc" ] || {
  printf "fatal: account Labwc config still contains X11 xinit asset: %s\n" "$account_home/.config/labwc/xinitrc" >&2
  exit 1
}
[ "$(grep -c "^collections = null\$" "$account_home/.config/vdirsyncer/config")" -eq 2 ] || {
  printf "fatal: account vdirsyncer config is missing explicit collections = null pairs: %s\n" "$account_home/.config/vdirsyncer/config" >&2
  exit 1
}
...
grep -q "pam_systemd.so class=greeter type=wayland desktop=cage" /etc/pam.d/greetd-greeter || {
  printf "fatal: greetd greeter PAM is missing the managed greeter session class\n" >&2
  exit 1
}
